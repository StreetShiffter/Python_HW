...
def test_mask_several(input_num, result_num):
...
def test_invalid_card_number(input_string):
...
def test_mask_account_several(input_num, result_num):
    assert get_mask_account(input_num) == result_num

@pytest.mark.parametrize("try_string", [
     "Номер карты",
     "!@#$%^&*()_+",
     "123"                      # Длина номера
])
def test_invalid_account_number(try_string):
    with pytest.raises(ValueError):
