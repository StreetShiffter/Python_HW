...
def test_transaction_descriptions(right_description: list[dict]) -> None:
    expected: list[str] = ["Перевод организации", "Перевод со счета на счет", "Отсутствует описание"]
    iterations: list[str] = list(transaction_descriptions(right_description))
...
def test_card_number_generator(generator_card: list[dict]) -> None:
    result: list[dict] = generator_card
    start: str = "1"
    stop: str = "5"
...
def test_card_number_generator_raises()-> None:
    with pytest.raises(ValueError):
        test_1: list[str] = list(card_number_generator(5, 2))
        test_2: list[str] = list(card_number_generator(10, 2))
        test_3: list[str] = list(card_number_generator(3, "five")) # type: ignore
        assert test_1
        assert test_2
        assert test_3
